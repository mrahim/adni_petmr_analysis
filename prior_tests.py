...
for key in regressor.keys():
    ...
    for k in range(7):
        ...
        for train, test in sss:
            ...
            for lmb in np.logspace(-3, 3, 50):
                pc = PriorClassifier(regressor[key], w_pet, lmb)
                pc.fit(x[...,k], y)
                #pc.fit(xtrain, y_train)                
                print lmb, pc.score(xtest, y_test)
            scores.append(pc.score(xtest, y_test))
        print '--'
        seeds_scores.append(scores)
    all_scores[key] = seeds_scores

np.savez_compressed(os.path.join(FEAT_DIR, 'regressors_scores_ad_nc_emci'),
