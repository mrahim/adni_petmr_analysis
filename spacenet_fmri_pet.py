# -*- coding: utf-8 -*-
"""
Created on Wed Feb 11 14:06:00 2015
...
a = np.copy(x)
### Spacenet regression with variable substitution
for k in range(7):
    
    x = a[...,k]
    lambda_ = .7
    w_pet = np.array(model)
    Y = y - (lambda_ * np.dot(x, w_pet.T))[:, 0]   
    
    spn = SpaceNetRegressor(penalty='smooth-lasso', eps=1e-1, 
                            mask=mask_img,
                            n_jobs=10, memory=CACHE_DIR)
                        
    imgX = array_to_niis(a, mask_img, k)
    spn.fit(imgX, Y)
    ### original weights
    w = spn.coef_ - lambda_ * w_pet
    ...
    plt.savefig(os.path.join(FIG_DIR, 'spn_output'+str(k)+'.png'))
    plt.savefig(os.path.join(FIG_DIR, 'spn_output'+str(k)+'.pdf'))
    ...
    plt.savefig(os.path.join(FIG_DIR, 'spn_output'+str(k)+'-diff.png'))
    plt.savefig(os.path.join(FIG_DIR, 'spn_output'+str(k)+'-diff.pdf'))